...
public class DataSet {
    ...
    public static Double euclidianDistance(HashMap<String, Double> a, HashMap<String, Double> b) {
    ...
    public Double calculateClusterSSE(HashMap<String, Double> centroid, int clusterNo) {
        ...
        for (int i=0; i<records.size(); i++) {
            if(records.get(i).clusterNo == clusterNo) {
                SSE += Math.pow(euclidianDistance(centroid, records.get(i).getRecord()), 2);
            }
        }
        return SSE;
    }
        ...
        for (int i=0; i<records.size(); i++) {
            if(!indicesOfCentroids.contains(i)) {
                ...
                for(int ind : indicesOfCentroids) {
                    double dist = euclidianDistance(records.get(i).getRecord(), records.get(ind).getRecord());
                    if(dist<minDist) {
                        minDist = dist;
                    }
                }
